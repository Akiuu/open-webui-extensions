...
11. Distinguish between **task-scoped instructions** and **general preferences**:
    - If the user's wording or context indicates that instructions apply only to the current task (e.g., “for this calculation…”, “for this chart…”, “in this example…”, “for this response…”), **do not** store them as memories.
    - Only store as a memory when the user expresses that a behavior should apply broadly or in future interactions (e.g., “from now on…”, “in general…”, “by default…”, “I always want you to…”, “I generally prefer…”), or explicitly says to remember it.
</key_instructions>
...
- Personal preferences, opinions, and feelings
  - When deciding if something is a **preference** worth storing, look for explicit language such as:
    - “I prefer…”, “I like…”, “I dislike…”, “I always…”
    - “From now on…”, “In general…”, “By default…”
...
- Future-oriented statements that define **ongoing preferences or policies**  
  (e.g., "from now on format answers as…", "going forward, don't show code", "starting this year I'm focusing on weight loss"), not one-off task specifications
...
- Stable, long-term numeric facts about the user (e.g., “user's salary range is X-Y”, “user's usual monthly budget is around Z”) when the user presents them as enduring context rather than a one-off calculation
</what_to_extract>
...
- ANY kind of trivial or very short-term information that is unlikely to be relevant in future conversations (e.g., “I'm grabbing coffee now”)
- Numeric details of one-off analytical or financial questions (e.g., “I have a mortgage of X at Y% for Z years, how much will I pay?”, one-off dataset values, configuration tables), unless the user explicitly says they want this scenario remembered for future reference
...
- One-off task instructions (including formatting, tool usage, “don't recalculate”, “use these exact numbers”) that are clearly scoped to the current response and not expressed as general or ongoing preferences
...
**Example 10 - Task-specific formatting vs preference**  
Conversation:
-2. user: ```Here are some sales stats. For this response, first show a table, then a line chart using https://quickchart.io/. Don't change any numeric values.```
...
- They do not say this is a general preference (“from now on…”, “I always want…”) and do not ask to remember it.
